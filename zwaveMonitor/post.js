...
//var dbPath = '/home/stephen/Projects/Django/myGates/mygatesdb'
   ...
   for(var nodeId in ZWaveAPIData.devices) {
      ...
      for(var instance in device.instances) {
         ...
         for(var cc in device.instances[instance].commandClasses) {
            //console.log("CC: " + cc + " " + device.instances[instance].commandClasses[cc].name);
...
function initDeviceEventHandlers() {
   ...
   for(var nodeId in ZWaveAPIData.devices) {
      ...
      for(var instance in device.instances) {
         ...
         if(instance == 0 && Object.keys(device.instances).length > 1) {
            console.log("Found multi instance: " + nodeId);
            continue;
         }
         ...
         for(var cc in device.instances[instance].commandClasses) {
            ...
            if(0x9c == cc) {
               ...
               for(var sense in device.instances[instance].commandClasses[cc].data) {
                  ...
                  if(0x5 == sense) {
                     ...
                     console.log(device.instances[0].commandClasses[0x9c].data[5].sensorState.value);
            ...
            if(0x30 == cc) {
               console.log("Binary sensor Handler");
               ...
               for(var sense in device.instances[instance].commandClasses[cc].data) {
                  ...
                  // Binary sensor
                  //
                  console.log("Sensor path: " + alarmpath);

                  ZWaveAlarm[alarmpath] = function(x, y) { 

                     var eventType = "Binary sensor event"
                     var eventData = null;
                     var device = ZWaveAPIData.devices[x];

                     if(true == device.instances[0].commandClasses[0x30].data[1].level.value) {
                        eventData = "level = true (Motion detected!)"
                        console.log("Binary sensor detecting motion!!");
                     } else {
                        eventData = "level = false (No Motion!)"
                        console.log("Binary sensor detecting No motion");
                     }
